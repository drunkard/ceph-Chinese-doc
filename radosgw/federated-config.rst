...
Ceph 从 0.67 Dumpling 起支持加入 :term:`Ceph 对象网关`\ 联盟，可加入多个辖\
区、或一个辖区内的多个域。

- **辖区（ Region ）：** 辖区是地理空间的\ *逻辑*\ 划分，它包含一个或多个域。\
  一个包含多个辖区的集群必须指定一个主辖区。
...
程运行在不同的地理时区、甚至不同的 Ceph 集群上，使用 Ceph 对象网关辖区功能和\
元数据同步代理都能实现全局统一的命名空间。如果你想多保留一个（或多个）数据副\
本，可以把一个辖区内的一或多个 Ceph 对象网关例程拆分到多个逻辑容器，然后配置 \
Ceph 对象网关域和数据同步代理，这样你就能多拥有一份或多份主域的数据副本了。额\
外的数据副本对于故障转移、备份和灾难恢复是很重要的。

如果你有低延时的网络连接（并非建议），可以把 Ceph 存储集群配置为联盟模式的单\
体集群。也可以在每辖区部署一套 Ceph 存储集群，然后把各存储池划分进各个域（典\
型部署）。如果资源充足、且冗余性要求严格，你也可以在每个域都部署一个独立的 \
Ceph 存储集群。
...
- **配置主辖区：** 这一段描述了如何配置起包含多个域的一个辖区，以及如何同步主\
  辖区内的主域和各二级域。
  
- **配置二级辖区：** 这一段描述了如何重复前一段再配置一个包含多个域的主辖区，\
  这样你就有了两个辖区，而且都实现了辖区内同步。最后，你会了解到如何配置元数\
  据同步代理，以此统一各辖区命名空间。
...
本段提供的典型步骤可帮你配置起包含两个域的辖区，此集群将包含两个网关守护进程\
例程——每域一个。此辖区将作为主辖区。
...
配置集群前，规划良好的辖区、域和例程名字可帮你更好地管理集群。假设此辖区代表\
美国，那我们就引用她的标准缩写。
...
你可以把整个辖区配置为一个 Ceph 存储集群，也可以把各个域都配置为一个 Ceph 存\
储集群。
...
.. note:: 按照以上步骤配置二级辖区时，需把 ``us-`` 替换为 ``eu-`` 。创建主辖\
   区和二级辖区\ **后**\ ，你一共会拥有四个用户。
...
.. note:: 按照以上步骤配置二级辖区时，需把辖区、存储池和域的名字都从 ``us`` \
   替换为 ``eu`` 。创建主辖区和二级辖区\ **后**\ ，你一共会有四条类似配置。


创建辖区
--------

#. 为 ``us`` 辖区创建个辖区配置文件，名为 ``us.json`` 。

   把下列实例的内容复制进文本编辑器，把 ``is_master`` 设置为 ``true`` ，用终\
   结点的全资域名替换 ``{fqdn}`` 。这样，主域就是 ``us-east`` ，另外 \
   ``zones`` 列表中还会有 ``us-west`` 。详情见\ `配置参考——辖区`_\ 。
   ::
...
如果你把辖区配置到不同的 Ceph 存储集群上，可以加 \
``--name client.radosgw-us-west-1`` 选项重复上述的第二、四、五步。也可以从初\
始网关例程导出辖区图，并按更新步骤导入。

.. note:: 按照以上步骤配置二级辖区时，需把 ``us`` 替换为 ``eu`` 。创建主辖区\
   和二级辖区\ **后**\ ，你一共会有两个辖区。
...
本段所述的典型步骤可帮你配置起一个拥有多个辖区的集群。配置一个跨辖区集群要求\
维护一个全局命名空间，这样分布于不同辖区的对象名就不会存在命名空间冲突问题了。

本段对\ `配置主辖区`_\ 里的步骤有所扩展，还更改了辖区名称、修改了一些步骤，详\
情见下文。
...
重复执行\ `配置主辖区`_\ 里的典型步骤，不同之处如下：

#. 按照\ `为二级辖区命名`_\ 而非\ `为主辖区命名`_\ 。
   
#. `创建存储池`_\ ，用 ``eu`` 取代 ``us`` 。

#. `创建密钥环`_\ 及对应密钥，并用 ``eu`` 取代 ``us`` 。如果你想用同样的密钥\
   环也可以，只要确保给此辖区（或辖区和域）创建好密钥就行。
...
#. `创建辖区`_\ ，用 ``eu`` 取代 ``us`` 。把 ``is_master`` 设置为 ``false`` ，\
   为保持一致性，在二级辖区里也要创建主辖区。
   ::
...
数据同步代理会把主域数据复制到二级域。一个辖区的主域会自动选择某辖区的二级域，\
并作为它的数据源。
...
一例程只需准备主机名和端口号即可（假设一辖区或域内的所有网关例程都访问同一 \
Ceph 存储集群）；配置文件（如 ``cluster-data-sync.conf`` ）里的以下这些选项\
要填上，还要给 ``log_file`` 指定一个日志文件。
...
数据同步代理会把主辖区内主域的元数据复制到二级辖区的主域。元数据包含网关用户\
和各桶，但不包含桶内的对象——为确保跨集群统一的命名空间。主辖区内的主域是数据\
源，它会自动选择二级辖区的主域。

.. image:: ../images/region-sync.png
   :align: center

按照与\ `多站点数据复制`_\ 相同的步骤，并把主辖区的主域作为源域、二级辖区的主域作为二级域。启动 ``radosgw-agent`` 时加上 ``--metadata-only`` 参数，这样它就只会复制元数据。例如：
::

	radosgw-agent -c inter-region-data-sync.conf --metadata-only

完成前述各步骤之后，你就会拥有一套包含两个辖区的集群，其内的主辖区 ``us`` 和\
二级辖区 ``eu`` 的命名空间是统一的。
