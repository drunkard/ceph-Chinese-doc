...
除非另有声明， Ceeph 的源代码会以 LGPL2.1 许可证发布。其条款\
位于\ `源码树顶极目录下的 COPYING 文件内`_\ 。

.. _`源码树顶极目录下的 COPYING 文件内`:
...
.. _Using ccache to speed up local builds:
...
See `Redmine 问题跟踪器`_ for a brief introduction to the Ceph Issue Tracker.
...
Before you start, you should know the `问题跟踪`_ number of the bug
...
must fork it first. Forking is what enables us to open a `GitHub 拉取请求`_.
...
the `Git 提交的实践真知`_ article at the `OpenStack 项目百科`_.

.. _`Git 提交的实践真知`: https://wiki.openstack.org/wiki/GitCommitMessages
.. _`OpenStack 项目百科`: https://wiki.openstack.org/wiki/Main_Page

Once your pull request (PR) is opened, update the `问题跟踪`_ by
...
commits in the PR are properly signed (see `补丁的提交`_) and a
``make check`` test.
...
Refer to the `测试`_ chapter for details.


.. _Integration tests AKA ceph-qa-suite:
...
Integration testing is discussed in more detail in the `测试`_ chapter.
...
by discussions on `IRC`_ and the `邮件列表`_.

修订你的 PR
-----------

While your PR is going through `测试`_ and `源码审核`_, you can
...
to change the `问题跟踪`_ status to "Resolved". Some issues may be
flagged for backporting, in which case the status should be changed to
"Pending Backport" (see the `补丁移植（ Backporting ）`_ chapter for details).
...
If you run into trouble, ask for help on `IRC`_ or the `邮件列表`_, or
...
.. _Deploy a cluster for manual testing:
...
.. _Testing - how to run s3-tests locally:
