...
- **归置组：** :ref:`autoscaler <pg-autoscaler>`
  ...
  有个适用各种场景以及存储池、副本、和每个 OSD 内目标 PG 副本\
  各种组合的便捷计算器，可在 :ref:`pgcalc` 中获得指导。
...
创建存储池前先看看 :ref:`rados_config_pool_pg_crush_ref` 。
在 Ceph 中央配置数据库里有个默认配置选项（名为 ``osd_pool_default_pg_num`` ），
用于确定每个新存储池的 PG 数量，在创建存储池、
却没给它单独指定 PG 数时就用这个值。这个值的默认数值可以更改。
关于每个存储池的归置组数量，见\ :ref:`setting the number of placement groups` 。

.. note:: 从 Luminous 版起，所有存储池都必须与使用它们的应用程序关联。
   详情见\ `关联存储池与应用程序`_\ 。
...
.. describe:: {pg-num}
   ...
   存储池拥有的归置组总数。关于如何计算合适的数值，参见
   :ref:`归置组`\ 。默认值对大多数系统都 **不合适** 。
...
.. describe:: {replicated|erasure}
   ...
   ``erasure`` （纠删，一种\
   :ref:`通用奇偶校验 RAID <ecpool>` 策略，类似 RAID6 但更加灵活）。
...
.. describe:: [crush-rule-name]
   ...
   :类型: String
   :是否必需: No.
   :默认值: 对于多副本存储池（ ``replicated`` ）来说，其规则由
            :confval:`osd_pool_default_crush_rule` 配置指定，此规则必须存在。
            对于纠删码存储池（ ``erasure`` ）来说，如果用的是 ``default``
            :ref:`纠删码配置 <erasure-code-profiles>`\ 那就是 ``erasure-code`` ，
            否则就是 ``{pool-name}`` 。如果此规则不存在，会悄悄地创建。

.. describe:: [erasure-code-profile=profile]

   仅适用于\ ``纠删``\ 存储池。让 Ceph 采用指定的\
   :ref:`纠删码配置 <erasure-code-profiles>`\ 。此配置必须是已经存在的配置，
   通过仪表盘定义、或调用 ``osd erasure-code-profile set`` 定义的。
   注意，在存储池创建以后，对其 EC 配置的更改\ *不会*\ 生效。要更改现有存储池的
   EC 配置，必须更改存储池，让它使用另外一个按需要定义好的 CRUSH 规则。
...
.. describe:: [expected-num-objects]
   ...
   为这个存储池预估的对象数。如果设置了此值，
   在创建存储池时就会拆分 PG ，以免运行时拆分文件夹导致延时增大。
...
每个存储池都必须先与应用程序关联才能使用。要用于 CephFS 的存储池、
或由 RGW 创建的存储池已经自动关联过了；计划用于 RBD 的存储池应该\
通过仪表盘或 ``rbd`` CLI 工具初始化（详情见\ `块设备命令`_\ ）。

对于其它用例，你可以手动关联存储池与应用程序名字（格式自由），执行下列命令：
...
每个存储池的配额都可以设置最大字节数、或每存储池最大对象数，执行下列命令：
...
否则 Ceph 监视器们会拒绝删除存储池。

详情见\ :ref:`监视器配置 <monitor-config-reference>`\ 。

如果有自定义规则不再使用或不再需要，应该删掉它们。

.. prompt:: bash $

   ceph osd pool get {pool-name} crush_rule

例如，假设自定义规则为 123 ，检查所有存储池看它们是否在用此规则，执行下列命令：
...
关于更新一个用户能力的说明，见 :ref:`修改用户能力 <modify-user-capabilities>` 。
...
.. describe:: compression_min_blob_size
   ...
   :描述: 设置可压缩数据块的最小尺寸：小于此值的数据块不会被压缩。此设置会覆盖下列全局配置：
...
.. note:: 用 ``ceph df detail`` 可以查看压缩存储池的统计信息。
   通过大多数 API 调用或 CLI 命令，压缩池中的对象将显示其原始未压缩时的大小。
   例如：直接放置在压缩 RADOS 存储池中却未进行 RGW 级别压缩的 RGW 桶，
   将通过 ``radosgw-admin`` 报告其未压缩的桶大小。 
...
.. describe:: min_size
   ...
   :描述: 为保证 PG 活跃且能处理 I/O 操作，要求设置的最小活跃副本数（或分片数），
          ...
          永久 OSD 故障这样的事件时，数据会丢失。更多信息见\ :ref:`ecpool` 。
   :类型: 整数
   :适用版本: ``0.54`` 及以上。
...
.. describe:: pg_num
   ...
   :描述: 指定存储池的 PG 总数。注意 PG autoscaler ，如果在指定存储池上启用了，可能会覆盖手动分配的数值。
   :类型: 整数
...
.. describe:: allow_ec_optimizations
   ...
   :描述: 允许为纠删码存储池实现性能和容量优化。这些优化专为 CephFS 和 RBD 载荷设计；
          RGW 载荷有大量小对象或小的随机访问读取，也可以从中受益。有大块顺序读、
          写操作的 RGW 载荷则受益甚微。详情见 :ref:`rados_ops_erasure_coding_optimizations` 。
   :类型: Boolean
...
.. describe:: target_max_bytes
   ...
   :描述: 启用了已废弃的分级缓存功能、且达到 ``max_bytes`` 阀值时， Ceph 就回写或赶出对象。
   :类型: 整数
...
.. describe:: scrub_min_interval
   ...
   :描述: 存储池 PG 两次成功的轻度洗刷之间的最小间隔秒数。如果这个存储池属性\
          还是默认值 ``0`` ，就采用中央配置库里的 ``osd_scrub_min_interval`` 。
   :类型: Double
   :默认值: ``0``
...
.. describe:: scrub_max_interval
   ...
   :描述: 存储池 PG 两次成功的轻度洗刷之间间隔的最大秒数。影响 overdue 属性，
          在洗刷调度器转储里会出现。如果默认值还是 ``0`` ，
          就采用中央配置库里的 ``osd_scrub_max_interval`` 数值。
   :类型: Double
   :默认值: ``0``
...
.. describe:: deep_scrub_interval
   ...
   :描述: “深度”洗刷存储池 PG 的间隔秒数。如果仍是默认值 ``0`` ，就采用中央配置库里的 ``osd_deep_scrub_interval`` 。
   :类型: Double
   :默认值: ``0``
...
.. note:: 一个处于降级模式的 PG ，其副本数小于规定值 ``pool size`` ，
   但仍可接受 I/O 请求。要设置一个 I/O 正常运作所需的最小副本数，
   应该用 ``min_size`` 选项。例如，执行下列命令：

.. prompt:: bash $

   ceph osd pool set data min_size 2

此命令可确保数据存储池里任何副本数小于 ``min_size`` （这里是 2 ）
的对象都不会收到 I/O 了。注意，在生产环境下设置 ``size``
为 ``2`` 或者 ``min_size`` 为 ``1`` 有丢数据的风险，
...
.. describe:: {peering_crush_bucket_count}
   ...
   此值与 ``peering_crush_bucket_barrier`` 一起使用，可以决定选定的 acting set
   内的一组 OSD 是否能相互建立对等互联，根据的是 actine set 中不同桶的数量。
...
.. describe:: {crush_rule}
   ...
   此存储池所用的 CRUSH 规则。存储池类型和 CRUSH 规则类型必须相匹配（ ``replicated`` 或 ``erasure`` ）。
...
.. describe:: {--yes-i-really-mean-it}
   ...
   需要使用该标记来确认您是否真的想绕过安全检查并为某个存储池设置值，
...
.. describe:: {crush_rule}
   ...
   退出 stretch 存储池模式后采用的 CRUSH 规则。存储池类型和
   CRUSH 规则类型必须相匹配（ ``replicated`` 或 ``erasure`` ）。
...
.. describe:: {min_size}
   ...
   退出 stretch 存储池模式后，能正常提供 I/O 服务必需的最低副本数。
