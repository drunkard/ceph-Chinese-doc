...
还有个中央配置库的 ``pg_autoscale_mode`` 选项，
控制着集群自最初设置好以来所创建存储池的自动伸缩模式。
用下列命令更改此配置：
...
您可以用 ``noautoscale`` 标志禁用或启用所有存储池的自动伸缩器 autoscaler 。
默认情况下，该标记设置为关闭 ``off`` ，
...
- **TARGET RATIO** （如果存在）是此存储池的预期存储量
  ...
  则 ``target_size_ratio`` 优先。注意，当 BIAS 值不为 1 时，
  尤其是对于 CephFS 元数据和 RGW 索引存储池，最好保持目标比率不变，
  因为同时调整二者，可能会由于双重调整反而得到不合适的 ``pg_num`` 值。
...
- **BIAS** 是用于手动调整存储池 PG 数的一个乘数，
  ...
  对于主要将数据存储在 omap 而非 RADOS 对象中的存储池而言，
  尤其是 RGW 索引和 CephFS / RBD EC 元数据存储池，这一点至关重要。
  当给存储池设置非 1.0 的 bias 值时，建议不要设置目标比率（ target ratio ）。

- **PG_NUM** 是当前与存储池相关联的 PG 数量，或者，
  如果正在进行 ``pg_num`` 变更，则是该存储池正要达到的 PG 数量。

- **NEW PG_NUM** （如果存在）是系统给当前存储池建议的 ``pg_num`` 值。
  它总是 2 的幂次，
  只有当建议值与当前值的差异超过伸缩阈值时才会出现。
  此阈值默认为配置的系数 ``3`` 。
  在调低时，仅使用配置的因子；
  而在扩展时，阈值会动态降低：
  如果推荐的 NEW PG_NUM 为 512 或 1024 ，则阈值设置为 1.0 ；
  如果推荐的 NEW PG_NUM 为 2048 ，则阈值设置为 2.0 。
  要调整这个倍数（在下面的示例中，
  它被改为 ``2`` ），执行下列命令：
...
Ceph 会考虑整个系统的总空闲空间、每个 OSD 的目标 PG 数量、
以及每个存储池中存储了多少数据，并相应地分配 PG 。
系统的做法比较保守，只有在当前 PG 数量（ ``pg_num`` ）\
与建议数量相差超过伸缩阈值时，才会对存储池进行更改。
在收缩时，只使用这个配置的因子；然而，在扩大时，
这个阈值会动态地降低：当推荐的 NEW PG_NUM 为 512 或 1024 时，
阈值会自动设置为 1.0 ；为 2048 时自动设置为 2.0 。
...
除了最小化的部署外，建议将该值设置为 200 。
值高于 500 可能会导致过多对等流量以及 RAM 占用量过大。

自动伸缩器（ autoscaler ）会分析存储池，并以子树为单位进行调整。
由于每个存储池可能映射到不同的 CRUSH 规则，
而每个规则可能会把数据摊派到不同的、还可能重叠的设备上，
因此 Ceph 将独立考虑分级结构中每个子树的利用率。
例如，映射到 ``ssd`` 类 OSD 的存储池和\
映射到 ``hdd`` 类 OSD 的存储池，
它们各自的最佳 PG 数量将取决于这两种不同类型设备的数量。
...
从而避免随后更改 ``pg_num`` 以及\
随之而来的重新归位数据带来的开销。
确保 PG 分布到所有可用的 OSD 上，这也有助于提升性能和数据一致性。
...
注意，大多数情况下，建议不要在同一存储池上同时设置 1.0 以外的 bias 值和目标比率。
对于元数据或 omap 相对多的存储池，可设置较高的 bias 值；
对于 RADOS 数据相对多的存储池，应设置目标比率。
...
如果给一个存储池设置了最小值，那么 Ceph 自己不会把这个存储池的
``pg_num`` 值缩减（也不建议您缩减）到配置值以下。
设置最小值的目的是为了给客户端 I/O 过程的并行数量设定一个下限，
即使存储池大部分是空的也是如此。

如果设置了最大值，则 Ceph 自己不会把 PG 数量增加
（也不建议您增加）到配置值以上。
...
如果你不在这个命令里指定 ``pg_num`` ，
集群会基于这个存储池内的数据量，用 PG 自动伸缩器自动配置这个参数，
（见 :ref:`pg-autoscaler` ）。

不过，创建时指定或不指定 ``pg_num`` 对以后\
集群是否会自动调整该参数没有任何影响。
...
在没有均衡器（ balancer ）的情况下，给大部分集群建议的
（但不是默认值）目标是每个 OSD 上大约 200 个 PG 副本。
打开均衡器且采用默认值时，最初每个 OSD 上应该有大约 50-70 个 PG 副本。
这个数值位于 ``ceph df`` 输出下的 ``PGS`` 一列，
而且，它不是这个集群的总 PG 数除以 OSD 数得出的。
...
如果 OSD #2 出现故障，那么归置组 #2 必须恢复对象副本（利用 OSD #3 ）。

PG 数量增加时，会产生几种后果。
新 PG 会被分配 OSD 。 CRUSH 函数的结果会发生变化，
这意味着现有 PG 中的某些对象会被复制到新 PG 中，并从旧的 PG 中移除。
...
性能以及数据均衡地分布到各 OSD 上都倾向于使用更多的 PG ；
而节约 CPU 资源和最小化内存用量则倾向于使用较少的 PG 。
在 Filestore OSD 废弃前，后者更为重要；因此，
而最新的集群采用 BlueStore OSD ，更倾向于前者，
于是把 ``mon_target_pg_per_osd`` 配置为 200-250 ，
把 ``mon_max_pg_per_osd`` 配置为 500 。
注意，后者仅作为故障保护机制，本身并不影响 ``pg_num`` 的计算。
...
这就是为什么 ``size=2`` 的多副本存储池和 ``m=1`` 的
EC 存储池有风险、且通常不建议使用的微妙原因。
...
集群中的每一个归置组都会对 OSD 和监视器产生内存、网络、处理器方面的需求。
这些需求必须随时得到满足，在恢复期间还会增加。
事实上，开发出 PG 的主要原因之一就是通过把对象聚集成可管理的尺寸来分担这种开销。
...
这里的\ **pool size （存储池副本数）**\ 是指多副本存储池的副本数或\
纠删码存储池的 K+M 之和。要获取这个和，运行命令
``ceph osd erasure-code-profile get`` 。

接下来，检验一下生成的基准值是否与您设计的 Ceph 集群一致，
即最大化\ :ref:`data durability`\ 和\ :ref:`object distribution`\ ，
并最大限度地减少\ :ref:`resource usage`\ 。
...
把所有存储池的 ``pg_num`` 之和凑成 2 的幂次既没有必要，也不容易。
...
以便得到合理的 PG 总量。重要的是，要确定出一个数量，
既能为每个 OSD 提供相对较低的差异，又不会额外增加\
对系统资源的负担或者使互联进程过于缓慢。

以这样一个集群为例，它拥有 10 个存储池、每个存储池有 512 个归置组，
分布在 10 个 OSD 上；即 5120 个 PG 散布在 10 个 OSD 上，
也就是平均每个 OSD 上有 512 个 PG ；如此配置，不会占用太多资源。然而，
如果集群有 1000 个存储池，且各存储池分别有 512 个 PG 分布在 10 个 OSD 上，
那么每个 OSD 就得处理 5 万多个归置组，
这个集群将明显需要更多资源和更多时间建立互联。
...
尽管如此，存储池创建后，如果没用 ``pg_autoscaler`` 去管理
``pg_num`` 数值，你仍然可以用下面的命令更改归置组数量：

.. prompt:: bash # 

   ceph osd pool set {pool-name} pg_num {pg_num}

从 Nautilus 版起，在归置组数量变动后，通过 PG autoscaler
或手动操作， Ceph 会自动且增量地逐步更新 ``pgp_num`` 。
管理员通常无需直接修改 ``pgp_num`` ，但可以通过
``watch ceph osd pool ls detail`` 来监控进度。
当 ``pg_num`` 发生变化时， ``pgp_num`` 的值会缓慢更新，
以便将拆分或合并 PG 的成本随时间分摊，从而尽可能地减少对性能的影响。 

增大一个存储池的 ``pg_num`` 会引起那个存储池中 PG 的分割，
但是只有当 ``pgp_num`` 增加以后，数据才会经由回填操作迁移到新 PG 里。

手动设置 ``pgp_num`` 参数是可能的，但很少恰当。 ``pgp_num`` 参数的数值\
应等于 ``pg_num`` 参数。要增加用于归置的 PG 数量，执行下列命令：

.. prompt:: bash #

   ceph osd pool set {pool-name} pgp_num {pgp_num}

如果你减少或增大一个存储池的 ``pg_num`` ， ``pgp_num`` 数值会自动调整 。
Ceph 从 Nautilus 版本起，如果没采用 ``pg_autoscaler`` ，
``pgp_num`` 会自动跟随 ``pg_num`` 的数值。
